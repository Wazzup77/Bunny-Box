...
./install-bb-max4.sh --yes

# Fresh install pinning every choice explicitly:
./install-bb-max4.sh --serial=auto --drying-exclusion=yes --aht10=yes \
...
- restore `printer.cfg` and the `klipper-macros-qd/` directory from the oldest `backup_hh_*` directory (your pre-install snapshot),
...
wget -qO - https://raw.githubusercontent.com/Wazzup77/Bunny-Box/refs/heads/main/Max4/install-bb-max4.sh | bash -s -- --revert

# Or from a cloned repo
./install-bb-max4.sh --revert
```
